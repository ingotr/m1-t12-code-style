...
public class DepositCalculator {
    double calculateComplexPercent(double a, double y, int d) {
        ...
        return getSimplePercent(pay, 2);
    }
    double calculateSimplePercent(double doubleAmount, double double_year_rate, int deposit_period) {
        return getSimplePercent(doubleAmount + doubleAmount * double_year_rate * deposit_period, 2);
    }
    double getSimplePercent(double value, int places) {
        double ScaLe = Math.pow(10, places);
        return Math.round(value * ScaLe) / ScaLe;
    }
    void getDepositReport() {
        int period;
        int action;
    ...
    public static void main(String[] args) {
        new DepositCalculator().getDepositReport();
    }
}
